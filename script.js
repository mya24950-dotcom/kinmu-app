...
let scheduleStickyHeader = null;
let scheduleStickyHeaderTable = null;
let scheduleStickyWrapper = null;
let scheduleStickyEventsBound = false;


/* 固定ヘッダーを削除 */
function removeScheduleStickyHeader() {

  if (scheduleStickyHeader) {
    scheduleStickyHeader.remove();
  }

  scheduleStickyHeader = null;
  scheduleStickyHeaderTable = null;
  scheduleStickyWrapper = null;

}


/* 画面上部の固定ヘッダーの高さを取得 */
function getScheduleTopOffset() {
...
/* 固定ヘッダーを作成 */
function createScheduleStickyHeader() {

  removeScheduleStickyHeader();
  ...
  stickyTable.id =
    "scheduleStickyHeaderTable";

  stickyTable.style.borderCollapse =
    "separate";

  stickyTable.style.borderSpacing =
    "0";

  stickyTable.style.tableLayout =
    "fixed";

  stickyTable.style.position =
    "relative";

  stickyTable.style.margin =
    "0";

  stickyTable.style.padding =
    "0";

  stickyTable.style.boxSizing =
    "border-box";
  ...
  /* 幅を合わせる */
  syncScheduleStickyColumns();


  /* 初期状態 */
  updateScheduleStickyHeader();


  /*
    イベントは一度だけ登録
  */
  if (!scheduleStickyEventsBound) {
...
function syncScheduleStickyColumns() {
  ...
  scheduleStickyHeaderTable.style.width =
    width + "px";

  scheduleStickyHeaderTable.style.minWidth =
    width + "px";

  scheduleStickyHeaderTable.style.maxWidth =
    width + "px";
  ...
  */
  ...
  for (
    ...
    if (w > 0) {
      ...
      stickyCols[i].style.width =
        w + "px";

      stickyCols[i].style.minWidth =
        w + "px";

      stickyCols[i].style.maxWidth =
        w + "px";
...
function updateScheduleStickyHeader() {
  ...
  if (!table || !thead) {
    ...
    scheduleStickyHeader.style.display =
      "none";
  ...
  */
  ...
  scheduleStickyHeader.style.left =
    wrapperRect.left + "px";


  scheduleStickyHeader.style.top =
    top + "px";


  scheduleStickyHeader.style.width =
    wrapperRect.width + "px";


  scheduleStickyHeader.style.height =
    headerHeight + "px";
  ...
  */
  ...
  scheduleStickyHeaderTable.style.transform =
    "translate3d(" +
    (
      -scheduleStickyWrapper.scrollLeft
    ) +
    "px, 0, 0)";
